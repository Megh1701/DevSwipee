...
const Sidebar = ({ avatar, isOpen = false, onClose = () => {}, light }) => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                ...
                // silent — notifications may not be available
            }
        };
    ...
    const unreadMatchCount = notifications.filter((n) => !n.read && n.type === "match").length;
    const unreadMessageCount = notifications.filter((n) => !n.read && n.type === "message").length;
    const unreadRequestCount = notifications.filter((n) => !n.read && (n.type === "invite" || n.type === "swipe")).length;

    const handleLinkClick = (type) => {
        onClose();
        const typesToClear = type === "request" ? ["invite", "swipe"] : [type];
        const unreadIds = notifications
            .filter((n) => !n.read && typesToClear.includes(n.type))
            .map((n) => n._id);
        if (unreadIds.length > 0) {
            setNotifications((prev) =>
                prev.map((n) => (typesToClear.includes(n.type) ? { ...n, read: true } : n))
            );
            unreadIds.forEach((id) => api.put(`/api/notifications/${id}/read`).catch(() => {}));
        }
    };
  ...
  onClick={onClose}
/>
            <div className={`${light ? "bg-[#ededed]" : "bg-black"} fixed left-0 top-0 z-[999] flex h-screen w-72 max-w-[85vw] -translate-x-full flex-col items-center justify-start gap-6 overflow-y-auto border-r border-dashed border-neutral-600 bg-dark px-4 pb-6 pt-36 transition-transform duration-300 md:w-1/4 md:translate-x-0 md:p-6 md:pt-36 ${isOpen ? "translate-x-0" : ""}`}>
                <button
                    type="button"
                    onClick={onClose}
                    className="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-md border border-neutral-700 text-text-dark md:hidden"
                ...
                <Link to="/home" id="home-nav-link" onClick={onClose}>
                    <button className="relative flex min-h-10 w-40 overflow-visible items-center justify-start gap-3 rounded-3xl border border-neutral-700 px-4 py-2 cursor-pointer md:w-36">
                ...
                <Link to="/swipes" id="swipes-nav-link" onClick={() => handleLinkClick("match")}>
                    <button className="relative flex min-h-10 w-40 overflow-visible items-center justify-start gap-3 rounded-3xl border border-neutral-700 px-4 py-2 cursor-pointer md:w-36">
                        ...
                        {unreadMatchCount > 0 && (
                            <span className="absolute -top-1.5 -right-1.5 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white shadow-[0_0_10px_rgba(239,68,68,0.8)] border-2 border-[#121212] z-10">
                ...
                <Link to="/messages" id="messages-nav-link" onClick={() => handleLinkClick("message")}>
                    <button className="relative flex min-h-10 w-40 items-center justify-start gap-3 rounded-3xl border border-neutral-700 px-4 py-2 cursor-pointer md:w-36">
                ...
                <Link to="/requests" id="requests-nav-link" onClick={() => handleLinkClick("request")}>
                    <button className="relative flex min-h-10 w-40 items-center justify-start gap-3 rounded-3xl border border-neutral-700 px-4 py-2 cursor-pointer md:w-36">
                ...
                <Link to="/ats-dashboard" id="ats-nav-link" onClick={onClose}>
                    <button className="relative flex min-h-10 w-40 items-center justify-start gap-3 rounded-3xl border border-neutral-700 px-4 py-2 cursor-pointer md:w-36">
                ...
                <button
                    ...
                    className="relative mt-auto mb-8 flex min-h-10 w-40 items-center justify-center gap-2 overflow-hidden rounded-3xl border border-dashed border-neutral-600 px-4 py-2 cursor-pointer transition-colors hover:border-blue-500 md:w-36"
