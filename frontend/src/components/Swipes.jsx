...
const Swipes = ({ light }) => {
  ...
  const handleFilter = (filterKey) => {
    // If the same filter is clicked again, revert to default "newest"
    ...
    setFilterModel(false); // close dropdown
  };
  ...
  const emptyMessage = () => {
    switch (activeFilter) {
      case "accepted":
        return "Ah, your request didn’t make it… the penguin feels you. 🐧💙"
      case "interested":
        return "No pending swipes ⏳";
      case "rejected":
        return "The search came up empty. Penguin walks on. 🐧"
      case "oldest":
      case "newest":
      default:
  ...
  const FilterItem = ({ icon, label, filterKey, onClick }) => (
    <button
      onClick={onClick}
      className="flex items-center gap-3 w-full px-3 py-2 cursor-pointer rounded-lg text-sm text-white hover:bg-white/10 transition"
    >
      {icon}
      {label}
  ...
  return (
    ...
    >
      <div className="relative flex min-h-screen flex-col justify-between overflow-x-hidden px-3 py-20 sm:px-4 sm:py-6 md:px-4 md:py-6">
        {/* ---------- HEADER & FILTER BUTTON ---------- */}
        <div className="relative flex items-center justify-between gap-3 md:justify-around">
          <h2 className={`truncate text-xl font-bold sm:text-2xl ${iconColor}`}>My Swipes</h2>
          ...
          <motion.div
           ...
           >
            <motion.img
              ...
              src={current.ownerId.avatar}
              className="aspect-square w-full rounded-xl object-cover"
            />
            <div className={`mt-3 h-1/2 space-y-2 rounded-2xl border p-3 text-neutral-500 sm:p-4 ${cardBorder}`}>
              <div className="flex items-center justify-between">
                <motion.h2 layoutId={`name-${current._id}`} className={`font-semibold ${light ? "text-black" : "text-white"}`}>
                  {current.ownerId.name}
                </motion.h2>
                <motion.div
                  layoutId={`status-${current._id}`}
                  className={`px-4 py-1 rounded-full text-xs font-bold border
                    ${getStatusStyles(current.status).bg}
                    ${getStatusStyles(current.status).text}
                    ${getStatusStyles(current.status).border}
                    ${getStatusStyles(current.status).glow}`}
                >
                  {swipeStatus[current.status].toUpperCase()}
                </motion.div>
              </div>
              <motion.p layoutId={`project-${current._id}`} className="text-sm">
                Project: {current.projectId.title}
              </motion.p>
              <p className="text-xs text-neutral-400">Stack: {current.projectId.stack}</p>
           <motion.p className="text-sm  whitespace-pre-line leading-relaxed h-36 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-neutral-500/40 scrollbar-track-transparent pb-[25%] [mask-image:linear-gradient(to_top,transparent_0%,black_80%)]"
            ...
            <div className="flex gap-3 pt-2">
              {current.projectId.githubUrl && (
                <a href={current.projectId.githubUrl} target="_blank" rel="noopener noreferrer"
              ...
              {current.projectId.liveDemoUrl && (
                <a href={current.projectId.liveDemoUrl} target="_blank" rel="noopener noreferrer"
        ...
        {filteredRequests.length === 0 ? (
          ...
          <motion.div
            key={`${activeFilter}-${page}`} // <- key changes when filter changes
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <motion.p layoutId={`project-${swipe._id}`} className="text-sm opacity-70">
                    Project: {swipe.projectId.title}
                  </motion.p>
                </div>
                <motion.div
                  layoutId={`status-${swipe._id}`}
                  className={`rounded-full border px-3 py-1 text-[11px] font-bold
                    ${getStatusStyles(swipe.status).bg}
                    ${getStatusStyles(swipe.status).text}
                    ${getStatusStyles(swipe.status).border}
                    ${getStatusStyles(swipe.status).glow}`}
                >
                  {swipeStatus[swipe.status].toUpperCase()}
                </motion.div>
              </motion.div>
            ))}
          </motion.div>
